...
public class UserRest {
	...
	public UserLinkListResource getUsersXML() {
		// Escribe tu código aquí {
		ServletUriComponentsBuilder builder = ServletUriComponentsBuilder.fromCurrentServletMapping();
		UserLinkListResource userLinkListResources = new UserLinkListResource();
		
		userLinkListResources.addLink(new Link(builder.path("/").build().toString(), "api"));
		userLinkListResources.addLink(new Link(builder.path("/user").build().toString(), "user"));

		for (User u : userService.getUsers()) {
			String user = u.getUsername();
			userLinkListResources.addUserLink(new Link(builder.path("").build().toString() + "/" + user, user));
		}
		// }
		return userLinkListResources;
	}
